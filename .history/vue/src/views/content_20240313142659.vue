...
onMounted(() => {
  ...
  axios.post("http://localhost:9090/order/picture", parsedData)
    .then(res => {
      function fetchImageData(parsedData: any) {
    return axios.post("http://localhost:9090/order/picture", parsedData)
        .then(res => {
            ...
            console.log(gallery); // 这里可以查看 gallery 对象是否正确更新
        ...
        .catch(error => {
            ...
            // 如果请求失败，返回一个空数组或其他适当的默认值
            return [];
        });
}
// 调用函数获取图片数据，并在外部处理
fetchImageData(parsedData)
    .then(images => {
        console.log(images); // 在外部打印获取到的图片数据
        console.log(gallery.images); // 在外部也可以访问到更新后的 gallery.images
    });
    })
});
