...
'https://res2.vmallres.com/pimages/uomcdn/CN/pms/202311/gbom/6941487293926/428_428_D50DE377227CAD2517913DFD62A62E8Emp.png'],
  selectedImage: ''
};
const price = ref(0)
const Nprice = ref(0)
...
onMounted(() => {
  ...
  axios.post("http://localhost:9090/order/picture", parsedData)
    .then(res => {
      ...
      res.data.data.forEach((item: { url: string; }) => {
        gallery.images.push(item.url);
      });
    })
    gallery.images = gallery.images.map(url => `'${url}'`);
  console.log(gallery);
});
//购买
const Purchase = () => {
