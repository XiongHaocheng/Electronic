...
// 构造要发送到后端的数据对象
const dataToSend = selectedRows.value.map(row => ({
  name: row.name, // 假设 name 是 Cart 对象中的一个属性
  // 其他属性也需要在这里添加
}));

// 添加 totalPrice 到数据对象
const totalPriceToSend = {
  totalPrice: totalPrice.value
};

// 将 totalPriceToSend 添加到 dataToSend 数组中
dataToSend.push(totalPriceToSend);

  axios.post('http://localhost:9090/cart/purchase', dataToSend)
  .then(response => {
