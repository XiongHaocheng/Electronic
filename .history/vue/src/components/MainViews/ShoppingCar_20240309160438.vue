...
// 
const date = {
  date: selectedProducts.map(product => product.date)
};
console.log(newList);
console.log(date);
 axios.post('http://localhost:9090/order/commit', newList,date)
 .then(response =>{
 })
 axios.post('http://localhost:9090/order/commit', date)
 .then(response =>{
 })
};
