...
>
  ...
  <template #footer>
    <div class="dialog-footer">
      <el-button @click="dialogVisible = false">q</el-button>
  ...
  <el-affix position="bottom" :offset="10">
    ...
    <el-col :span="2">
      <el-button type="danger" style="width: 130px;height: 60px;margin-top: -9px;margin-bottom: -9px;" @click.prevent="cartPurchase(scope.$index)">购买</el-button>
    </el-col>
  </el-row>
...
const fetchCartData = async () => {
  try {
    ...
    tableData.value = response.data; // 将返回的购物车数据赋值给tableData
...
const cartPurchase = async (index: number) => {
  const purchaseRow = tableData[index]; // 获取要购买的行数据
  console.log(purchaseRow);
  tableData.splice(index, 1);
  try {
    // 发送购买请求至后端
    await axios.post('http://localhost:9090/cart/purchase', { data: purchaseRow });
    console.log('购买成功');
  } catch (error) {
    console.error('购买失败:', error);
    // 如果购买失败，您可能需要将行数据重新插入到tableData中
    // tableData.splice(index, 0, purchaseRow);
  }
};
