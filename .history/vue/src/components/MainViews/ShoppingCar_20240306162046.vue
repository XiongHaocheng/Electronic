...
async cartPurchase(index) {
  const purchaseRow = this.tableData[index]; // 获取要购买的行数据
  console.log(purchaseRow);
  this.tableData.splice(index, 1);
  try {
    // 发送购买请求至后端
    await axios.post('http://localhost:9090/cart/purchase', purchaseRow);
    console.log('购买成功');
  } catch (error) {
    console.error('购买失败:', error);
    // 如果购买失败，您可能需要将行数据重新插入到tableData中
    // this.tableData.splice(index, 0, purchaseRow);
  }
}
